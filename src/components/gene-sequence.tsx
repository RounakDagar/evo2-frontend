...
export function GeneSequence({
            ...
            if (isDraggingStart) {
                if (!isNaN(currentEndNum)) {
                    if (currentEndNum - newPosition + 1 > maxViewRange) {
                        onStartPositionChange(String(currentEndNum - maxViewRange));
                ...
                if (!isNaN(currentStartNum)) {
                    if (newPosition - currentStartNum + 1 > maxViewRange) {
                        onEndPositionChange(String(currentStartNum + maxViewRange));
    ...
    }, [isDraggingStart, isDraggingEnd, isDraggingRange, geneBounds, startPosition, endPosition, onStartPositionChange, onEndPositionChange, maxViewRange, onSequenceLoadRequest])
        ...
        for (let i = 0; i < sequenceData.length; i += BASES_PER_LINE) {
            ...
            lines.push(
                <div key={i} className="flex">
                    <div className="text-gray-500 mr-2 w-20 text-right select-none">
    ...
    // console.log("Gene Detail inside GeneSequence:", geneDetail);
    // console.log("Genomic Info:", geneDetail?.genomicinfo?.[0]);
    // console.log("Strand value:", geneDetail?.genomicinfo?.[0]?.strand);
                        ...
                        <div className="space-y-4">
                            <div className="relative">
                                ...
                                >
                                    ...
                                    {/* Start handle */}
                                    <div 
                                        className="absolute top-1/2 flex h-6 w-6 -translate-x-1/2 -translate-y-1/2 cursor-grab items-center justify-center rounded-full border-2 border-[#3c4f3d] bg-white shadow active:cursor-grabbing"
                                        style={{
                                            left: `${sliderValues.start}%`,
                                        }}
                ...
                <div className="w-full rounded-md bg-[#e9eeea]/50 p-3">
                    ...
                    ? (
                        ...
                        <div className="h-64 overflow-x-auto overflow-y-auto">
                            <pre className="font-mono text-xs leading-relaxed items-center">
                                {formattedSequence}
                            </pre>
                        </div>
                    ) : (
