...
export function VariantComparisionModal({
  comparisionVariant,
  onClose,
}: {
  comparisionVariant: ClinvarVariant | null;
  onClose: () => void;
}) {
  if (!comparisionVariant || !comparisionVariant.evo2Result) {
    return null;
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-lg bg-white">
        ...
        {/* Modal Content */}
        <div className="p-5">
          {comparisionVariant && comparisionVariant.evo2Result && (
                  ...
                  <div>
                    <div className="space-y-2">
                      ...
                      <div className="flex">
                        ...
                        <span className="text-xs">
                          {comparisionVariant.variation_type}
                        </span>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex">
                        ...
                        <span className="font-mono text-xs">
                          {(() => {
                            const match =
                              comparisionVariant.title.match(/(\w)>(\w)/);
                            ...
                            return comparisionVariant.title;
                          })()}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center">
                        ...
                        <a
                          ...
                          href={`https://www.ncbi.nlm.nih.gov/clinvar/variation/${comparisionVariant.clinvar_id}`}
                          target="_blank"
                        >
                          {comparisionVariant.clinvar_id}
                        </a>
                ...
                <div className="rounded-md border border-[#3c4f3d]/10 bg-white p-4">
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div className="rounded-md bg-[#e9eeea]/50 p-4">
                      <h5 className="mb-2 flex items-center gap-2 text-xs font-medium text-[#3c4f3d]">
                        <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[#3c4f3d]/10">
                          <span className="animation-duration:[5s] h-3 w-3 animate-ping rounded-full bg-[#3c4f3d]"></span>
                        </span>
                        ClinVar Assessment
                      </h5>
                      <div className="mt-2">
                        <div
                          className={`w-fit rounded-md px-2 py-1 text-xs font-normal ${getClassificationColorClasses(comparisionVariant.classification)}`}
                        >
                          {comparisionVariant.classification ||
                    ...
                    <div className="rounded-md bg-[#e9eeea]/50 p-4">
                      <h5 className="mb-2 flex items-center gap-2 text-xs font-medium text-[#3c4f3d]">
                        <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[#3c4f3d]/10">
                          <span className="animation-duration:[5s] h-3 w-3 animate-ping rounded-full bg-[#de8246]"></span>
                        </span>
                        Evo2 Prediction
                      </h5>
                      <div className="mt-2">
                        <div
                          className={`flex w-fit items-center gap-1 rounded-md px-2 py-1 text-xs font-normal ${getClassificationColorClasses(comparisionVariant.evo2Result.prediction)}`}
                        >
                          <Shield className="h-3 w-3" />
                          {comparisionVariant.evo2Result.prediction}
                        </div>
                      </div>
                      ...
                      <div className="mt-3">
                        ...
                        <div className="text-sm font-medium">
                          {comparisionVariant.evo2Result.delta_score.toFixed(6)}
                        </div>
                        <div className="text-xs text-[#3c4f3d]/60">
                          {comparisionVariant.evo2Result.delta_score < 0
                      ...
                      <div className="mt-3">
                        ...
                        <div className="mt-1 h-2 w-full rounded-full bg-[#e9eeea]/80">
                          <div
                            className={`h-2 rounded-full ${comparisionVariant.evo2Result.prediction.includes("pathogenic") ? "bg-red-600" : "bg-green-600"}`}
                            style={{
                              width: `${Math.min(100, comparisionVariant.evo2Result.classification_confidence * 100)}%`,
                            }}
                          ></div>
                        </div>
                        <div className="mt-1 text-right text-xs text-[#3c4f3d]/60">
                          {Math.round(
                            comparisionVariant.evo2Result
                  ...
                  <div className="mt-4 rounded-md bg-[#e9eeea]/20 p-3 text-xs leading-relaxed">
                    <div className="flex items-center gap-2">
                      <span
                        className={`flex h-5 w-5 items-center justify-center rounded-full ${comparisionVariant.classification.toLowerCase() === comparisionVariant.evo2Result.prediction.toLowerCase() ? "bg-green-100" : "bg-yellow-100"}`}
                      >
                        {comparisionVariant.classification.toLowerCase() ===
                        comparisionVariant.evo2Result.prediction.toLowerCase() ? (
                      ...
                      <span className="font-medium text-[#3c4f3d]">
                        ...
                        comparisionVariant.evo2Result.prediction.toLowerCase()
                          ...
                          : "Evo2 prdecition differs from ClinVar classification"}
                      </span>
                    </div>
                  </div>
                </div>
